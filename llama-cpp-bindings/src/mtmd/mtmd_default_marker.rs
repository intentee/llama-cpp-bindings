...
/// the llama.cpp library; surfacing the error keeps the failure explicit rather than
...
mod tests {
    ...
    fn returns_non_empty_marker() {
        let marker = mtmd_default_marker().expect("llama.cpp marker must be valid UTF-8");
