...
fn tool_call_markers_are_absent_for_a_model_without_a_chat_template(
    fixture: &LlamaFixture<'_>,
) -> Result<()> {
    let markers = fixture.model.tool_call_markers()?;

    assert!(
        markers.is_none(),
        "an encoder model carries no chat template, so it advertises no tool-call markers; got: {markers:?}"
    );
